    ...
    */
    ...
    workerSpace.on('connection', function (client) {
        ...
        client.on('get', (data, callback) => {
            ...
            //Brian Chen, doing one job at a time
            var found = false;
            for(var i = 0; i < queue.length; i++) {
                if (queue[i].status == 'in progress' || queue[i].status == 'background-in-progress') {
                    found = true;
                    break;
                }
            }
            ...
            const nextPendingJob = queue.find((job) => {
                ...
                if (!found && job.type == 'queue' && job.status == 'pending' && (job.host === undefined || job.host == data.host)) {
        ...
        client.on('done', (inboundJob, callback) => {
            ...
            if (job.error !== undefined) {
                console.error('Job failed on worker', { id: job.id, type: job.type, host: inboundJob.host, platform: inboundJob.platform, background: inboundJob.background }, job.error);
                job.status = 'failed';
                if (job.background !== true){
                    job.emitter.emit('error', job.error);
                }
